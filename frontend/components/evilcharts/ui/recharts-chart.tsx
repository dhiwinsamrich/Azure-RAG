...
interface ChartContainerProps
  extends
    // `onResize` is also declared by ResponsiveContainer below with a
    // different signature; the div's version is dropped so the two Pick
    // sources do not collide (recharts 3.x).
...
function ChartContainer({
  ...
  children,
  footer,
  // These belong to ResponsiveContainer, not to the wrapping <div>. Pulling
  // them out of the rest spread stops them landing on a DOM node (React would
  // warn, and `onResize` has an incompatible signature) and forwards them to
  // the container that actually implements them.
  onResize,
  aspect,
  debounce,
  height,
